...
class CrudRepository
{
    ...
    public function productsGrid()
    {
        ...
        $grid->add('<img src="'.url('/').'/images/products/{{ $a_img }}" height="25" width="25">', 'Thumbnail');

        /*$grid->add('<img src="/images/products/{{ $image1 }}" height="25" width="20">', 'Image1');
        $grid->add('<img src="/images/products/{{ $image2 }}" height="25" width="20">', 'Image2');
        $grid->add('<img src="/images/products/{{ $image3 }}" height="25" width="20">', 'Image3');
        $grid->add('<img src="/images/products/{{ $image4 }}" height="25" width="20">', 'Image4');
        $grid->add('<img src="/images/products/{{ $image5 }}" height="25" width="20">', 'Image5');*/

        //$grid->add('<img src="/images/products/{{ $b_img }}"height="25" width="20">', 'Side');
        $grid->add('quantity', 'Qty');
        $grid->add('price', 'Price');
        $grid->add('product_weight', 'Product Weight (Grams)');
        $grid->add('shipping_weight', 'Shipping Weight (Grams)');
        $grid->edit(url('/').'/backend/products/edit');
        $grid->link(url('/').'/backend/products/edit', "New Products", "TR");
        $grid->orderBy('id', 'asc');
        $grid->paginate(10);
        return $grid;
    }
    ...
    public function productsEdit(){
        ...
        $edit->add('product_weight', 'Product Weight (Grams)', 'text');
        $edit->add('shipping_weight', 'Shipping Weight (Grams)', 'text');

        $edit->add('a_img', 'Thumbnail', 'image')->move('images/products/')->fit(500, 685)->preview(120, 80);
        ...
        $edit->add('image1', 'Image1', 'image')->move('images/products/')->fit(500, 685)->preview(120, 80);
        $edit->add('image2', 'Image2', 'image')->move('images/products/')->fit(500, 685)->preview(120, 80);
        $edit->add('image3', 'Image3', 'image')->move('images/products/')->fit(500, 685)->preview(120, 80);
        $edit->add('image4', 'Image4', 'image')->move('images/products/')->fit(500, 685)->preview(120, 80);
        $edit->add('image5', 'Image5', 'image')->move('images/products/')->fit(500, 685)->preview(120, 80);
    ...
    public function ordersGrid()
    {
        ...
        /*$grid->add('<a href="'.url('/').'/backend/products/edit?show={{ $products->product_id }}">{{ $products->name }}</a>', 'Product');*/
        $grid->add('size', 'Size');
        $grid->add('<img src="'.url('/').'/images/products/{{ $img }}" height="25" width="25">', 'Image');
        $grid->add('color', 'Color');
        $grid->add('quantity', 'Qty');
        $grid->add('amount', 'Amount');
        $grid->add('<a title="Update order status" href="/dash2/backend/order-status/{{$id}}">{{$status}} [Update]</a>', 'Status');
        $grid->edit(url('/').'/backend/orders/edit');
        $grid->link(url('/').'/backend/orders/edit', "New Order", "TR");
        $grid->orderBy('id', 'asc');
        $grid->paginate(10);
        return $grid;
    }
    ...
    public function ordersEdit()
    {
        ...
        $edit->add('quantity', 'Qty', 'text');
        /*$edit->add('status', 'Status', 'select')->options(array('New'=>'New', 'Dispatched'=>'Dispatched', 'Transit'=>'Transit', 'Delivered'=>'Delivered'));*/
        $edit->add('amount', 'Amount', 'text');
        $edit->link(url('/').'/backend/order-status/?modify='.$_GET['modify'], "Change Status", "TR");
        $edit->link(url('/').'/backend/orders', "Back", "TR");
        return $edit;
    }
