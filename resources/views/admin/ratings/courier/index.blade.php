            ...
            @if(count($ratings)>0)
                @foreach ($ratings as $key=>$rating)
                    ...
                    <tr class="bg-info @if($rating->seen == null) bg-seen @endif">
                        ...
                        <td>{{!empty($rating->courier_order)?$rating->courier_order->created_at:'N/A'}}</td>
                        <td>{{$rating->deliveried}}</td>
                        <td>{{$rating->created_at}}</td>
                        <td>
                            <a href="{{url("/backend/courier-orders/ratings/$rating->id")}}" class="btn btn-primary"><i class="fa fa-eye"></i></a>                            {{--<a href="{{route('cuisines.show',$cuisine->id)}}" class="btn btn-success"><i class="fa fa-eye"></i></a>--}}
